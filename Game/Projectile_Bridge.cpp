...
void Projectile_Bridge::PostHit(TileMap* &tile_map, std::vector<Player*> &players, unsigned int owner_index, std::vector<Explosion*> &explosions) {
    ...
    for (int y = 0; y < bridge_height; ++y) {
        for (int x = 0; x < bridge_length; ++x) {
            ...
            if (direction == -1 && tile_map->GetTile(p.x, p.y).status > 0 && p.x < tile_coords.x - 2) {
                break;
            } else if (direction == 1 && tile_map->GetTile(p.x, p.y).status > 0 && p.x > tile_coords.x + 2) {
                break;
            }
            tile_map->WriteStatus(p, 2);
        }
    }
}
